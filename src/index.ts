...
try {
    ...
    console.error(
        "Please ensure servers.json exists in either the root directory or src/ directory and is properly formatted.",
    );
    process.exit(1);
}
...
async function sendLog(serverId: string, embed: EmbedBuilder) {
    ...
    try {
        ...
        if (!logChannel || !logChannel.isTextBased()) {
            console.log(
                `⚠️  Log channel not found or not text-based in ${serverConfig.name}`,
            );
            return;
        }
        ...
        if (!textChannel.permissionsFor(client.user!)?.has("SendMessages")) {
            console.log(
                `⚠️  No permission to send messages in log channel for ${serverConfig.name}`,
            );
            return;
        }
...
async function registerCommands() {
    ...
    try {
        ...
        console.log(
            "⏳ Note: Global commands can take up to 1 hour to sync across all servers.",
        );
        ...
        console.log(
            "   Commands will be available in all servers within an hour.",
        );
    } catch (error) {
...
client.once("clientReady", async () => {
    ...
    for (const server of serversConfig.servers) {
        ...
        const logStatus = server.logChannelId
            ? `📝 Log: ${server.logChannelId}`
            : "⚠️  No log channel";
        if (guild) {
            // Check bot permissions
            const botMember = await guild.members.fetch(client.user!.id);
            const hasAdminPerms = botMember.permissions.has(
                PermissionFlagsBits.Administrator,
            );
            const hasBanPerms = botMember.permissions.has(
                PermissionFlagsBits.BanMembers,
            );
            const permStatus = hasAdminPerms
                ? "👑 Admin"
                : hasBanPerms
                  ? "✅ Can Ban"
                  : "❌ Cannot Ban";
            console.log(
                `  ✔️  ${server.name} (${server.id}) - ${permStatus} - ${logStatus}`,
            );
        } else {
            console.log(
                `  ❌ ${server.name} (${server.id}) - Bot not in this server!`,
            );
        }
    }
...
async function handleUnbanCommand(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        for (const configServer of serversConfig.servers) {
            ...
            try {
                ...
                if (
                    ...
                    console.log(
                        `  ❌ Missing ban permission in ${configServer.name}`,
                    );
                    continue;
                }
                ...
                await guild.members.unban(
                    userId,
                    `[BanSync Unban] ${reason} | Requested by ${interaction.user.tag}`,
                );
                ...
                console.log(
                    `  ❌ Failed to unban from ${configServer.name}: ${errorMessage}`,
                );
            }
        }

        // Calculate stats
        const unbannedCount = results.filter(
            (r) => r.success && !r.wasNotBanned,
        ).length;
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `Processed unban for user ID \`${userId}\` across ${serversConfig.servers.length} servers.`,
            )
            .addFields(
                {
                    name: "User ID",
        ...
        await interaction.editReply({
            content:
                "❌ An error occurred during the unban process. Check the logs for details.",
        });
    }
}
...
async function handleSyncCommand(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `Synchronized ${stats.totalBans} unique bans across ${serversConfig.servers.length} servers.`,
            )
            .addFields(
                {
                    name: "✅ New Bans Applied",
                    value: stats.newBans.toString(),
                    inline: true,
                },
                {
                    name: "🔄 Already Banned",
                    value: stats.alreadyBanned.toString(),
                    inline: true,
                },
                {
                    name: "❌ Errors",
        ...
        await interaction.editReply({
            content:
                "❌ An error occurred during the sync process. Check the logs for details.",
        });
    }
}
    ...
    for (const configServer of serversConfig.servers) {
        ...
        try {
            ...
            if (isBanProcessing(banInfo.userId, guild.id)) {
                ...
                console.log(
                    `  ⏭️  Skipping ${configServer.name} (already processing)`,
                );
                continue;
            }
            ...
            console.log(
                `  ✅ Banned ${banInfo.userTag} in ${configServer.name}`,
            );
...
client.on("guildBanAdd", async (ban) => {
    ...
    if (!serverMap.has(sourceGuild.id)) {
        console.log(
            `⚠️  Ban detected in non-configured server: ${sourceGuild.name} (${sourceGuild.id})`,
        );
        return;
    }
    ...
    if (isUserSyncing(bannedUser.id)) {
        console.log(
            `⚠️  Skipping ban - user ${bannedUser.tag} is currently being synced`,
        );
        return;
    }
    ...
    if (isBanProcessing(bannedUser.id, sourceGuild.id)) {
        console.log(
            `⚠️  Skipping BanSync ban to prevent loop: ${bannedUser.tag} in ${sourceServer.name}`,
        );
        return;
    }
    ...
    if (banReason.startsWith("[BanSync]")) {
        console.log(
            `⚠️  Skipping BanSync ban to prevent loop: ${bannedUser.tag}`,
        );
        return;
    }
    ...
    for (const configServer of serversConfig.servers) {
        ...
        try {
            ...
            console.log(
                `  ❌ Failed to ban in ${configServer.name}: ${errorMessage}`,
            );
        }
    }
    ...
    if (failureCount === 0) {
        ...
        console.log(
            `  ✅ Synced to ${successCount} server${successCount !== 1 ? "s" : ""}`,
        );
        ...
        for (const result of results) {
            ...
            const status = result.alreadyBanned
                ? " (already banned)"
                : result.error
                  ? ` (${result.error})`
                  : "";
...
async function sendBanReportToAll(
    ...
    if (failureCount === 0) {
        embed
            .setDescription(
                `User **${bannedUser.tag}** has been banned across all servers.`,
            )
            .addFields(
                {
                    name: "User",
                    value: `<@${bannedUser.id}> (\`${bannedUser.id}\`)`,
                    inline: false,
                },
        ...
        embed
            .setDescription(
                `User **${bannedUser.tag}** ban sync completed with some issues.`,
            )
            .addFields(
                {
                    name: "User",
